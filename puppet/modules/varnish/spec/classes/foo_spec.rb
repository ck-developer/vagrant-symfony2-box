...
describe 'varnish' do
  ...
  describe 'Test standard installation with monitoring and firewalling' do
    ...
    it 'should monitor the process' do
      content = catalogue.resource('monitor::process', 'varnish_process').send(:parameters)[:enable]
      content.should == true
    end
    it 'should place a firewall rule' do
      content = catalogue.resource('firewall', 'varnish_tcp_42').send(:parameters)[:enable]
      content.should == true
    end
  end
  ...
  describe 'Test customizations - source' do
    let(:params) { {:source => "puppet://modules/varnish/spec" , :source_dir => "puppet://modules/varnish/dir/spec" , :source_dir_purge => true } }

    it 'should request a valid source ' do
      content = catalogue.resource('file', 'varnish.conf').send(:parameters)[:source]
      content.should == "puppet://modules/varnish/spec"
    end
    it 'should request a valid source dir' do
      content = catalogue.resource('file', 'varnish.dir').send(:parameters)[:source]
      content.should == "puppet://modules/varnish/dir/spec"
    end
    it 'should purge source dir if source_dir_purge is true' do
      content = catalogue.resource('file', 'varnish.dir').send(:parameters)[:purge]
      content.should == true
    end
  end
  ...
  describe 'Test service autorestart', :broken => true do
    it 'should automatically restart the service, by default' do
      ...
      content.should == 'Service[varnish]{:name=>"varnish"}'
    end
  end
  ...
  describe 'Test Puppi Integration' do
    ...
    it 'should generate a puppi::ze define' do
      content = catalogue.resource('puppi::ze', 'varnish').send(:parameters)[:helper]
  ...
  describe 'Test Monitoring Tools Integration' do
    ...
    it 'should generate monitor defines' do
      content = catalogue.resource('monitor::process', 'varnish_process').send(:parameters)[:tool]
  ...
  describe 'Test Firewall Tools Integration' do
    ...
    it 'should generate correct firewall define' do
      content = catalogue.resource('firewall', 'varnish_tcp_42').send(:parameters)[:tool]
  ...
  describe 'Test OldGen Module Set Integration' do
    ...
    it 'should generate puppi resources ' do 
      content = catalogue.resource('puppi::ze', 'varnish').send(:parameters)[:ensure]
